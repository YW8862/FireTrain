...
| 技术 | 版本 | 选型理由 |
| --- | --- | --- |
| YOLOv8 | 8.0+ | 预训练模型，ONNX 格式推理，检测精度高 |
| ONNX Runtime | 1.20+ | 高性能模型推理引擎 |
...
**核心结论：需要自行训练的模型主要是 YOLOv8，并需导出为 ONNX 格式用于生产环境。**

| 分类 | 项目 | 是否需要训练 | 难度 | 说明 |
| --- | --- | --- | --- | --- |
| 必训（推荐） | YOLOv8 目标检测微调 + ONNX 导出 | 需要 | 中等 | 用于提升本场景（教室/走廊/光照变化/遮挡）下的检测稳定性，导出 ONNX 后推理速度提升 20-30% |
| 可不训（直接用） | MediaPipe Pose | 不需要 | 低 | 直接调用 Google 现成方案，重点在关键点映射、角度计算、阈值调参 |
| 可不训（规则） | 规则引擎与评分逻辑 | 不需要 | 低 - 中 | 属于规则设计与工程实现，不是机器学习训练 |
| 可选提升 | 时序动作识别模型 | 可选 | 中高 - 高 | 可提升智能化，但需视频片段标注与时序模型训练，周期风险较高 |

**1) 必训部分：YOLOv8 目标检测微调与 ONNX 导出**

- **训练阶段**：
  - 使用 Ultralytics YOLOv8 进行迁移学习
  - 从 `yolov8n.pt` 或 `yolov8s.pt` 开始微调
  - 训练完成后得到 `.pt` 格式权重文件
  
- **导出阶段**：
  - 将训练好的 `.pt` 模型导出为 ONNX 格式（`.onnx`）
  - ONNX 格式优势：
    - 跨平台部署，无需 PyTorch 依赖
    - 推理速度更快，内存占用更低
    - 支持量化优化（INT8/FP16）
  ...
  - 导出命令示例：
    ```bash
    yolo export model=runs/detect/fire_extinguisher_train/weights/best.pt format=onnx opset=12
    ```
  
- **生产部署**：
  - 使用 ONNX Runtime 加载 `.onnx` 模型进行推理
  - 相比原始 PyTorch 模型：
    - 启动速度更快（无需加载完整框架）
    - 推理延迟降低 20-30%
    - 内存占用减少约 40%
...
- AI 内聚：YOLOv8 训练后导出 ONNX 格式，MediaPipe、规则引擎作为内部模块调用，不额外拆服务
...
| 第 2 周 | 预训练模型测试、数据采集、ONNX 模型导出验证 | 模型测试报告、ONNX 模型文件 |
...
| 项目 | 内容 |
| --- | --- |
**训练对象**: YOLOv8（从 `yolov8n.pt` 迁移学习），训练后导出为 ONNX 格式（`.onnx`）
...
| Day 3 | 首轮训练（`yolov8n`，小批次）+ ONNX 导出测试 | 首版权重文件、训练日志、ONNX 模型 |
| Day 4 | 验证误检漏检并补采样本 | 错误样本清单、补充数据集 |
| Day 5 | 第二轮训练与参数微调 + ONNX 性能验证 | 改进版权重、对比结果、ONNX 性能报告 |
| Day 6 | 接入后端推理流程联调（基于 ONNX Runtime） | 接口可调用、可视化检测结果 |
...
| 参数 | 建议值 | 说明 |
| --- | --- | --- |
| 模型 | yolov8n (训练后导出为 ONNX) | 轻量、训练快，适合毕设周期，ONNX 格式推理更快 |
...
| 问题 | 参考回答 |
| --- | --- |
| 为什么选择 YOLOv8 + ONNX？ | YOLOv8 是较新的目标检测模型，精度高、速度快。导出为 ONNX 格式后，可实现跨平台部署、推理速度提升 20-30%、内存占用降低 40%，更适合生产环境。 |
| 姿态估计为什么用 MediaPipe？ | MediaPipe 无需训练即可使用，CPU 可运行，适合本科毕业设计的资源限制，且精度满足应用需求。 |
| 评分规则如何保证客观性？ | 评分基于几何计算和规则引擎，所有标准可量化，避免主观判断，且规则透明可解释。 |
| 系统实时性如何保证？ | 采用轻量级模型（yolov8n）、ONNX Runtime 高性能推理、视频降采样处理、关键帧分析，保证单帧处理时间 < 100ms。 |
