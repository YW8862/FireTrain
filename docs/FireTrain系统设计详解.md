...
6. 支持多训练类型动态配置，不同训练类型可拥有独立的步骤、提示语和评分维度。
...
- 动态引导：页面内的步骤说明、操作提示语根据选择的训练类型动态展示，不再硬编码。
- 强引导：页面内直接展示标准步骤，降低用户误操作概率。
...
在系统迭代过程中，统计分析页曾出现“点击进入后等待时间较长”的问题。结合前后端实现排查后，系统确认该问题并非单点故障，而是由前端首屏资源加载与后端统计查询重复聚合共同导致。

前端层面的主要瓶颈有两点：

- 统计页在路由懒加载时同时引入了 `echarts` 大型图表库，导致用户点击页面入口后，需要先等待页面脚本和图表资源一起完成加载。
- 页面初始阶段会同时请求个人统计、趋势统计和步骤分析三个接口，用户在视觉上容易感受到“页面尚未进入”或“进入后整体空白等待”。

针对该问题，前端做了以下优化：

1. 将 `echarts` 改为动态按需加载，避免在页面跳转前阻塞路由首屏渲染。
2. 将统计页改为“先进入页面、后异步加载数据”的模式，使页面框架和标题先渲染出来，再逐步填充统计内容。
3. 将整页统一加载遮罩改为局部加载状态，分别对概览区、趋势图区和步骤分析区独立显示 loading，降低用户感知等待成本。
4. 首屏优先请求统计概览接口，使训练次数、平均分等核心信息优先可见，再补充图表内容。

后端层面的主要瓶颈有两点：

- 统计页原始实现中，前端首屏需要调用多个统计接口，而这些接口内部都会重复执行按用户查询训练记录的逻辑。
- 服务层原本主要采用“先查询该用户全部训练记录，再在 Python 中逐项遍历和聚合”的方式完成统计，随着训练记录增多，数据库 I/O 和服务层计算开销都会同步上升。

为此，后端针对统计链路进行了进一步优化：

1. 新增统一的 `/stats/overview` 聚合接口，用于一次性返回统计页首屏所需的个人概览、趋势数据和步骤分析结果，减少前端重复请求。
2. 将个人统计中的总训练数、已完成训练数、平均分、最佳分、总训练时长和最近训练时间下推到数据库侧使用聚合函数计算，而不是先拉全量对象再在内存中求值。
3. 将趋势统计改为按日期分组的数据库聚合查询，直接得到每天的训练次数、平均分和最佳分，再由服务层补齐无数据日期。
4. 将步骤分析改为只提取已完成训练记录中的 `step_scores` JSON 字段，不再把整条训练记录对象及无关字段全部加载到内存中。

经过这一轮改造，统计页的处理链路从“多次请求 + 多次全量查询 + 多次重复聚合”，优化为“单次首屏请求 + 数据库聚合 + 按需字段读取”。这种优化虽然没有改变统计结果的业务语义，但显著改善了统计页首屏访问体验，也为后续论文中的性能分析、系统优化章节提供了典型案例。
...
### 11.3.1 面向统计分析页的聚合策略优化

统计模型在设计上不仅服务“数据展示”，还服务“查询性能控制”。在统计页优化过程中，系统进一步明确了三类不同的统计数据来源：

1. 个人概览类统计  
适合直接由数据库完成聚合，例如总训练数、平均分、最高分、总时长和最近训练时间。

2. 趋势类统计  
适合由数据库按日期分组完成初步聚合，再由服务层补齐空日期并整理为前端图表所需格式。

3. 步骤分析类统计  
由于依赖 `step_scores` 中存储的步骤级 JSON 数据，因此无法像普通数值字段一样完全依赖数据库完成通用聚合。系统在这一部分采取了折中策略，即只查询必要的 `step_scores` 字段，再在服务层进行步骤分、成功率和建议文本计算。

这种设计体现了一个重要思想：并不是所有统计都必须在数据库层完成，也不是所有统计都应在应用层完成，而应根据字段结构与聚合复杂度选择最合适的处理层级。

对于 FireTrain 而言，这种“数据库聚合 + 服务层轻量加工”的混合统计模式，既保证了性能，也保留了步骤级训练分析的灵活性。
...
系统在迭代过程中已经验证出一条有效的工程优化路线：先通过实际使用发现性能瓶颈，再从前端资源加载、接口组织方式、数据库聚合策略和字段读取粒度四个层面逐层优化。

以统计页为例，系统已经完成了以下典型优化：

- 图表库按需动态加载
- 统计页先渲染框架再异步加载数据
- 多接口请求收敛为单一概览接口
- 数据库端承担概览统计与趋势统计聚合
- 步骤分析仅按需读取 `step_scores`

这说明 FireTrain 不仅能实现业务功能，也具备持续进行性能调优和架构演进的能力。这一点对于毕业设计来说同样重要，因为它体现了系统并非一次性演示原型，而是具备可维护、可优化、可扩展的工程属性。
...
5. 以多训练类型动态配置实现业务弹性扩展，支持不同训练类型独立定义步骤与提示语。
