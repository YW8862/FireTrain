...
async def setup_test_data(db_session):
    """准备测试数据（供所有 TestStatisticsService / TestStatisticsAPI 用）"""
    session = db_session
...
@pytest.fixture
def client_fixture():
    """带鉴权覆盖的 HTTP 测试客户端：默认当作 setup_test_data 创建的用户"""
    from app.core.security import get_current_user_id

    user_id_holder = {"value": 1}
...
class TestStatisticsAPI:
    ...
    async def test_get_personal_statistics_api(self, client_fixture, setup_test_data):
        """测试个人统计 API"""
        user = setup_test_data

        response = client_fixture.get("/api/stats/personal")
    ...
    async def test_get_training_trend_api(self, client_fixture):
        """测试训练趋势 API"""
        response = client_fixture.get("/api/stats/trend?days=7")
    ...
    async def test_get_step_analysis_api(self, client_fixture):
        """测试步骤分析 API"""
        response = client_fixture.get("/api/stats/step-analysis")

        assert response.status_code == 200
        data = response.json()
        assert "step_analysis" in data

    async def test_get_statistics_overview_api(self, client_fixture):
        """测试统计概览 API"""
        response = client_fixture.get("/api/stats/overview?days=7")
